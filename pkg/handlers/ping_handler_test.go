...
func TestPingHandler_BotMessage(t *testing.T) {
	...
	assert.NotPanics(t, func() {
		handlers.PingHandler(session, message, &logger, "")
	})
}

func TestPingHandler_UserMessage(t *testing.T) {
	...
	// Execute - should try to send "Pong!" message
	assert.NotPanics(t, func() {
		handlers.PingHandler(session, message, &logger, "")
	})
}

func TestPingHandler_WithCommand(t *testing.T) {
	// Setup
	logger := zerolog.Nop()
	session := &discordgo.Session{}
	session.State = &discordgo.State{}
	...
	assert.NotPanics(t, func() {
		handlers.PingHandler(session, message, &logger, "test-command")
	})
}
